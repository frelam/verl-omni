...
DEFAULT_REWARD_SERVER_COMMAND = "vllm serve {model} --host {host} --port {port} --block-size 128 --gpu-memory-utilization 0.85 --max-model-len 40000"
...
def _maybe_launch_reward_server(args: argparse.Namespace):
    ...
    if hasattr(torch, "npu") and torch.npu.is_available():
        ...
        env["CUDA_VISIBLE_DEVICES"] = str(args.reward_gpu)  # 兼容部分上层框架
...
async def _generate_split(args: argparse.Namespace, split: str) -> Path:
    ...
    pipe = StableDiffusion3Pipeline.from_pretrained('/home/zzq/lhy/ckpt/stable-diffusion-3.5-medium', torch_dtype=dtype)
    pipe.to(args.device)
...
def main():
    ...
    parser.add_argument(
        ...
        dest="reward_gpu",
        help=(
            "Physical device index for the vLLM reward server (via ASCEND_RT_VISIBLE_DEVICES or CUDA_VISIBLE_DEVICES). "
            "Default: 0, or forced to 0 when only one accelerator is visible."
        ),
    )
    parser.add_argument(
        "--image-gpu",
        ...
        dest="image_gpu",
        help=(
            "Device index for image generation when --device is not set. "
            "Default: 0 with one visible card, else 1 (second card)."
        ),
    )
